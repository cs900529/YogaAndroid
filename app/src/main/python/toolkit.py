...
def treePoseRule(roi, tips, sample_angle_dict, angle_dict, point3d):
    ...
    for key, _ in roi.items():
        ...
        #if angle_dict[key] == -1:
        #    continue
        if key == 'RIGHT_KNEE':# or key == 'LEFT_HIP':
            #tolerance_val = 5
            min_angle = 170 #sample_angle_dict['LEFT_KNEE']-tolerance_val
            max_angle = 180 #sample_angle_dict['LEFT_KNEE']+tolerance_val
            if angle_dict[key] == -1:
                continue
...
def plankPoseRule(roi, tips, sample_angle_dict, angle_dict, point3d):
    ...
    for key, value in roi.items():
        ...
        if key == side + 'EYE':
            ...
            tolerance_val = 10
            min_angle = 60#sample_angle_dict[key]-tolerance_val
            max_angle = 85#sample_angle_dict[key]+tolerance_val
            if angle_dict[key] == -1 :
               continue
            if angle_dict[key]>=min_angle and angle_dict[key]<=max_angle:
            ...
            tolerance_val = 5
            min_angle = 165#sample_angle_dict[key]-tolerance_val
            #max_angle = 170#sample_angle_dict[key]+tolerance_val
            ...
            tolerance_val = 5
            min_angle = 160#sample_angle_dict[key]-tolerance_val
            if angle_dict[key] == -1 :
               continue
            if angle_dict[key]>=min_angle:
                roi['RIGHT_KNEE'] = True
...
def reversePlankPoseRule(roi, tips, sample_angle_dict, angle_dict, point3d):
    ...
    for key, _ in roi.items():
        ...
        if key == f"{side}_ELBOW":
            ...
            tolerance_val = 10
            min_angle = 60#sample_angle_dict[key]-tolerance_val
            max_angle = 80#sample_angle_dict[key]+tolerance_val
            if angle_dict[key] == -1 :
               continue
            if angle_dict[key]>=min_angle and angle_dict[key]<=max_angle:
                roi["LEFT_SHOULDER"] = True
...
def ChildsPoseRule(roi, tips, sample_angle_dict, angle_dict, point3d):
    ...
    for key, _ in roi.items():
        ...
        if key == 'NOSE':
            ...
            left_hip_x,_,_,left_hip_vi =  (point3d[AngleNodeDef.LEFT_HIP])
            right_hip_x,_,_,right_hip_vi =  (point3d[AngleNodeDef.RIGHT_HIP])
            if nose_x < MIN_DETECT_VISIBILITY : 
                roi[key] = False
